...
def make_and_upload_video(script, voice_module, isVertical, progress_func):
    ...
    def logger(prog_str):
        progress_func(progress_counter / num_steps, f"Creating video - {progress_counter} - {prog_str}")
    videoEngine.set_logger(logger)
    
    # 生成视频
    for step_num, step_info in videoEngine.makeContent():
        progress_func(progress_counter / num_steps, f"Creating video - {step_info}")
        progress_counter += 1
